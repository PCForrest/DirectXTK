...
local function ensureShadersForDirectXTK(prj)
...
local function deleteShadersForDirectXTK(prj)
	if (prj.name == "DirectXTK") then
...
p.override(m.elements, "project", function(base, prj)
	...
	table.insertafter(calls, m.importExtensionTargets, deleteShadersForDirectXTK)
	table.insertafter(calls, m.importExtensionTargets, ensureShadersForDirectXTK)
	return calls
end)
